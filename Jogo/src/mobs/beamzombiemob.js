 ...
 * ------------------------------------------------------------------
 * Queres mexer neste mob? Pontos principais:
 * 1) Tamanho global: no sistema de inimigos (`BEAMZOMBIE_SCALE`).
 * 2) Tamanho das pernas: `buildLeg()` (thighGeo, kneeGeo, shinGeo, bootGeo).
 * 3) Cabeça/olho: `skullGeo`, `visorGeo`, `eyeGeo` e `eyeMesh.position`.
 * 4) Braço-canhão: secção "Braço direito — canhão de plasma"
 *    - dimensão do cano: `barrelGeo`
 *    - orientação do cano: `barrel.rotation`
 *    - posição do bocal: `muzzle.position`
 * 5) Adicionar mais olhos/sensores:
 *    - duplica o bloco de `eyeMesh` e altera `position`.
 * 6) Animações:
 *    - marcha: `applyWalkPhase()` (swing, rotação pernas/braços)
 *    - brilho: `plasmaMat.emissiveIntensity` e `eyeMat.emissiveIntensity`
 */
...
export function createBeamZombieMob() {
  ...
  barrel.position.set(0.2, -0.36, 0.18);
  rightArm.add(barrel);
  ...
  for (let i = 0; i < 3; i++) {
    ...
    coil.position.set(0.2, -0.22 - i * 0.14, 0.16);
  ...
  muzzle.position.set(0.26, -0.66, 0.46);
  rightArm.add(muzzle);
  ...
  function applyWalkPhase(phase, moving) {
    ...
    barrel.rotation.z = 0.12 + Math.sin(_glowT * 1.1) * 0.03;
