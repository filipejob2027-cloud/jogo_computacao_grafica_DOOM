 ...
 * ------------------------------------------------------------------
 * 1) Tamanho global do Reaper: `REAPER_SCALE` em `systems/enemies.js`.
 * 2) Pernas: função `buildLeg()` (thighGeo/kneeGeo/shinGeo/clawGeo).
 * 3) Cabeça e olhos:
 *    - crânio: `skullGeo`
 *    - mandíbula: `jawGeo`
 *    - olhos: bloco das "orbitas oculares" (socketGeo e position).
 * 4) Braços e foices: `buildScytheArm()` (upperGeo/lowerGeo/bladeGeo).
 * 5) Se quiseres adicionar mais olhos/chifres:
 *    - duplica os blocos `forEach` na secção da cabeça.
 * 6) Animação: `applyWalkPhase()` (swing, postura agressiva, pulsos emissivos).
 */
