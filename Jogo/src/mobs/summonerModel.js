 ...
 * ------------------------------------------------------------------
 * Modelo procedural complexo (Summoner).
 * 1) Tamanho global: `SUMMONER_SCALE` em `systems/enemies.js`.
 * 2) Materiais: secção "Materials" (skinMat, boneMat, eyeMat...).
 * 3) Pernas: função `buildLeg(side)` com segmentos orientados por pontos.
 * 4) Cabeça/olhos/chifres: secções de cabeça e luzes dos olhos.
 * 5) Para adicionar olhos:
 *    - duplica criação de olhos (mesh + point light) e ajusta posições.
 * 6) Animação principal: função `update(dt, moving)` devolvida no final.
 */
