 ...
 * GUIA RÁPIDO
 * ------------------------------------------------------------------
 * Responsável por construir o mundo (paredes/chão/teto) e por dizer se
 * uma posição é bloqueada.
 *
 * Onde mexer:
 * - Texturas/materiais de paredes/chão: `makeWallTexture()` e `makeFloorTexture()`.
 * - Geometria do mundo: loop em `createWorld()` (wallGeom, floorGeom, ceil).
 * - Portão do nível 3: `buildLevel3DoorMesh()` + `registerLevel3DoorMesh()`.
 * - Ácido do nível 2: `buildAcidMeshes()` / `clearAcidMeshes()`.
 * - Colisões: `isWall(wx, wz)` (inclui bloqueio do portão).
 */
