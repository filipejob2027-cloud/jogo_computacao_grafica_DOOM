 ...
 * GUIA RÁPIDO
 * ------------------------------------------------------------------
 * Movimento/olhar/salto + stamina.
 *
 * Onde mexer:
 * - Sensibilidade do rato: `movementX/Y * 0.002`.
 * - Altura do salto: `JUMP_VEL` e `GRAVITY`.
 * - Velocidade base/sprint: `speed` e consumo de stamina.
 * - Colisão do jogador: `radius` e checks `isWall(...)`.
 * - "Tem de saltar": `player.jumpOffset` (0 no chão; >0 no ar).
 */
export const keys = {};
...
export function updatePlayer(ctx) {
  ...
  // Em ortho (câmara de cima), o movimento por yaw é confuso.
  // Aqui o WASD passa a ser em eixos do mundo (W = norte, S = sul, A = oeste, D = este).
  const topDown = state.cameraMode === 'ortho' || (camera && camera.isOrthographicCamera);
  const fwd = topDown
