...
export function createCombat(ctx) {
   ...
   * ----------------------------------------------------------------
   * Este sistema gere:
   * - tiros/ataques do jogador (raycasts)
   * - dano nos inimigos
   * - mortes + drops
   * - lógica de fim de onda e progressão
   *
   * Onde mexer:
   * - Dano das armas: `systems/weapons.js` (configs) e multiplicador `state.playerDamageMult`.
   * - Critério de "onda limpa": `enemies.every((e) => !e.alive)` em `killEnemy()`.
   * - Final de nível: blocos em `killEnemy()` (nível 1/2/3 chamam callbacks).
   */
  const {
    ...
    onLevel3CorridorCleared, onLevel3BossDefeated, onLevelBossDefeated
  } = ctx;
  ...
  function shoot() {
    ...
    if (state.weapon === 'axe') {
      ...
      for (const enemy of enemies) {
        ...
        const dist = new THREE.Vector3(enemy.x - cam.position.x, 0, enemy.z - cam.position.z).length();
    ...
    if (enemies.every((e) => !e.alive)) {
      ...
      if (state.currentLevel === 3) {
        if (clearedWave === 1) {
          state.wave = 2;
          updateHUD();
          showWaveMsg('CORREDORES LIMPOS! PROCURA A CHAVE');
          if (onLevel3CorridorCleared) onLevel3CorridorCleared();
          return;
        }
        if (clearedWave === 2) {
          showWaveMsg('BOSS DERROTADO! NIVEL COMPLETO');
          if (onLevel3BossDefeated) onLevel3BossDefeated();
          return;
        }
      }

      if (
        (state.currentLevel === 1 || state.currentLevel === 2) &&
        clearedWave > 1 &&
        clearedWave % 3 === 0
      ) {
        showWaveMsg('BOSS DERROTADO! NIVEL COMPLETO');
        if (onLevelBossDefeated) onLevelBossDefeated(state.currentLevel, clearedWave);
        return;
      }
