 ...
 * GUIA RÁPIDO
 * ------------------------------------------------------------------
 * HUD (vida/armadura/mortes/arma/munição/stamina/dificuldade) e mensagens.
 *
 * Onde mexer:
 * - Face do HUD (sprites): `healthToFaceIndex()` e `STATUS_FACE_DIR`.
 * - Mensagens no ecrã: `showWaveMsg(text)` (tempo/estilo).
 * - Barra de munição: `updateAmmoBar()` (pips + fracção).
 * - Feed de mortes: `addKillMsg()` (frases, duração).
 */
