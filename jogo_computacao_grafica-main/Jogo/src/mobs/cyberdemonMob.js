 ...
 * ------------------------------------------------------------------
 * 1) Tamanho global: `CYBER_SCALE` / `BOSS_CYBER_SCALE` em enemies.
 * 2) Pernas: função `buildLeg()` (thighGeo/kneeGeo/shinGeo/hoofGeo).
 * 3) Cabeça e olhos:
 *    - cabeça: `skullGeo`, `browGeo`, `snoutGeo`, `jawGeo`
 *    - olhos: bloco `[-0.42, 0.42].forEach(...)`.
 * 4) Braços:
 *    - esquerdo (carne): bloco `leftArm`
 *    - direito (canhão): bloco `rightArm` + anéis do canhão.
 * 5) Chifres: função `buildHorn()`.
 * 6) Animação de marcha: `applyWalkPhase()`.
 */
